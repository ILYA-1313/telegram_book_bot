#модуль с клавиатурой для закладок пользователя
...
def create_bookmarks_keyboard(*args: int) -> InlineKeyboardMarkup:
    #Инициализируем объекты клавиатуры
    kb_builder = InlineKeyboardBuilder()
    #Наполняем клавиатуру кнопками-закладками в порядке возрастания
    for button in sorted(args):
        kb_builder.row(InlineKeyboardButton(text=f"{button}-"
                                                 f"{book[button][:100]}",
...
def create_edit_keyboard(*args: int) -> InlineKeyboardMarkup:
    #Создаем объекты клавиатуры
    kb_builder = InlineKeyboardBuilder()
    #Наполняем клавиатуру кнопками-закладками в порядке возрастания
    for button in sorted(args):
        kb_builder.row(InlineKeyboardButton(text=f"{LEXICON['del']}"
                                                 f"{button} - {book[button][:100]}",
                                            callback_data=f"{button}del"))
        #Добавляем в конце клавиатуры кнопку "Отменить"
        kb_builder.row(
